...
def is_valid_asn(asn:str):
    try:
        ...
        if isinstance(asn, int) and 1 <= asn <= 4294967295:
...
def validate_moas(as_list):
    try:
        ...
        err_str = "Invalid MOAS event. The right format for a MOAS event is moas-<unix_timestamp>-<asn1_asn2_...>"
        raise ValidationError(err_str)
...
def validate_submoas(as_list):
    try:
        ...
        err_str = "Invalid SUBMOAS event. The right format for a SUBMOAS event is submoas-<unix_timestamp>-<victim-asn1_victim-asn2_...>=<attacker-asn1_attacker-asn2...>"
        raise ValidationError(err_str)
...
def validate_defcon(as_list):
    try:
        ...
        err_str = "Invalid DEFCON event ID. The right format for a DEFCON event is defcon-<unix_timestamp>-<victim-asn1_victim-asn2_...>"
        raise ValidationError(err_str)
...
def validate_edges(as_list):
    try:
        ...
        err_str = "Invalid edges event ID. The right format for a edges event is edges-<unix_timestamp>-<asn1_asn2>"
        raise ValidationError(err_str)
